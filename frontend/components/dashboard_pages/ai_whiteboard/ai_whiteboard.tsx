...
import { Tldraw, TLUiOverrides, toRichText, AssetRecordType , getHashForString, TLAssetStore, TLBookmarkAsset, uniqueId} from 'tldraw';
...
export default function AIWhiteboard() {
  ...
  const renderMermaid = useCallback(async () => {
    ...
    try {
      ...
      mermaidContainerRef.current.innerHTML = '<div class="text-red-500">Error rendering diagram. Please check your syntax.</div>';
    }
  }, [mermaidCode]);
  ...
  const handleInsertDiagram = useCallback(() => {
    if (renderedSVG && editorRef.current) {
      try {
        ...
        const svgBase64 = `data:image/svg+xml;base64,${btoa(unescape(encodeURIComponent(renderedSVG)))}`;
...
//     // don't need this , already has inferDarkMode in the tldraw component
